...
import { Package, FileText, Trash2, Users, X, LayoutDashboard } from 'lucide-react';
import { useRights } from '../context/UserRightsContext';

function Sidebar({ isOpen, setIsOpen }) {
  // Idagdag ang 'rights' sa kukunin mula sa context
  ...
  const navItems = [
    ...
    { name: 'Deleted Items', path: '/deleted-items', icon: Trash2,          show: isAdminOrAbove },
    // I-base natin ang Admin menu sa specific na right value (ADM_USER)
    { name: 'Admin',         path: '/admin',         icon: Users,           show: rights.ADM_USER === 1 }, 
  ];
  ...
  return (
    <>
      ...
      {/* Sidebar Panel */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white/70 backdrop-blur-2xl border-r border-white/40 shadow-2xl transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="h-20 flex items-center justify-between px-6 border-b border-slate-200/50">
          <div className="flex items-center gap-3">
            ...
            <span className="font-extrabold text-xl text-slate-900 tracking-tight">Product Management</span>
          </div>
          <button
            onClick={() => setIsOpen(false)}
